...
const UChart: React.FC = () => {
  ...
  const chartData = {
    ...
    datasets: [
      ...
      ...(chartOptions.showSigma1 ? [
        { label: '+1σ', data: oneSigmaValues.map((pointSigma) => centerLine + pointSigma), borderColor: 'rgba(255, 205, 86, 0.6)', borderDash: [2, 2], borderWidth: 1, pointRadius: 0, fill: false },
        { label: '-1σ', data: oneSigmaValues.map((pointSigma) => Math.max(0, centerLine - pointSigma)), borderColor: 'rgba(255, 205, 86, 0.6)', borderDash: [2, 2], borderWidth: 1, pointRadius: 0, fill: false },
      ] : []),
      ...(chartOptions.showSigma2 ? [
        { label: '+2σ', data: oneSigmaValues.map((pointSigma) => centerLine + 2 * pointSigma), borderColor: 'rgba(255, 159, 64, 0.6)', borderDash: [3, 3], borderWidth: 1, pointRadius: 0, fill: false },
        { label: '-2σ', data: oneSigmaValues.map((pointSigma) => Math.max(0, centerLine - 2 * pointSigma)), borderColor: 'rgba(255, 159, 64, 0.6)', borderDash: [3, 3], borderWidth: 1, pointRadius: 0, fill: false },
      ] : []),
  ...
  const options: ChartOptions<'line'> = {
    ...
    plugins: {
      legend: { position: 'top' },
      title: { display: true, text: chartOptions.title || 'U Chart (Defects per Unit)', font: { size: 16 } },
      tooltip: {
        callbacks: {
          label: (context) => {
    ...
    scales: {
      ...
      y: { title: { display: true, text: chartOptions.yAxisLabel || 'Defects per Unit' }, min: 0 },
    },
  };
